  ...
  actions: {
    async getList(payload: ProductModule.Filter): Promise<void> {
      try {
        ...
        const response: ProductModule.ProductListResponse = !payload.search
          ? await ApiService.instance().axios.get(
              Constant.PRODUCT_LIST + `?limit=${payload.limit}&skip=${payload.skip}`,
            )
